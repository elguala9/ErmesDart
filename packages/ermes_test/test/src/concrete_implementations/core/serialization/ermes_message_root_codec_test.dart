...
void testErmesMessageRootCodec() {
  group('buildMessageRoot', () {
    ...
    test('produces a plaintext MessageRoot when no cipher is registered',
      ...
      final root = buildMessageRoot(
        MessageType.data(MessageData(id: 1, data: data)),
        'peer-x',
      );
    ...
    test('the integrity hash is computed over the plaintext, independent '
      ...
      final plainRoot = buildMessageRoot(
        MessageType.data(MessageData(id: 1, data: data)),
        'peer-x',
      );
      ...
      final encryptedRoot = buildMessageRoot(
        MessageType.data(MessageData(id: 1, data: data)),
        'peer-x',
      );

      expect(
        encryptedRoot.integrityCheckValue,
        equals(plainRoot.integrityCheckValue),
      );
    });
  });
  ...
  group('decodeMessageEnvelope', () {
    ...
    test('decodes a valid legacy (v1) plaintext MessageRoot', () {
      ...
      final result = decodeMessageEnvelope(
        objectToUint8Array(root),
        'peer-x',
        {},
      );
    ...
    test('build-then-decode round trip recovers the original message '
      ...
      final built = buildMessageRoot(
        MessageType.data(MessageData(id: 5, data: data)),
        'peer-x',
      );
