  ...
  group('ErmesSignalingServerSubscriptions', () {
    ...
    group('subscribeToPeer', () {
      ...
      test('supports subscribing to multiple independent peers', () {
        ...
        expect(subscriptions.subscribedPeers, unorderedEquals(['peer-a', 'peer-b']));
      });
    });
    ...
    group('incoming event handling', () {
      test('a valid published signal is cached and forwarded to the '
        ...
        expect(subscriptions.cachedSignals['peer-a']?.epochTimestampStartConversation,
            equals(_nowEpoch + 100));
        expect(received?.epochTimestampStartConversation, equals(_nowEpoch + 100));
      });
      ...
      test('a newer signal (higher start timestamp) overwrites the cache',
          () {
        subscriptions.subscribeToPeer('peer-a');
        final peerNostr = InMemoryNostrSignaling('peer-a', store, subs);
        peerNostr.publish(utf8.encode(_signalAt(100).toString()));
        peerNostr.publish(utf8.encode(_signalAt(150).toString()));
      ...
      test('an older signal (lower start timestamp) never regresses the '
        ...
        final peerNostr = InMemoryNostrSignaling('peer-a', store, subs);
        peerNostr.publish(utf8.encode(_signalAt(150).toString()));
        peerNostr.publish(utf8.encode(_signalAt(50).toString()));
