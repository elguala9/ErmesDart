  ...
  group('ErmesSignalingServerListeners', () {
    ...
    group('notifySignal', () {
      test('does nothing when no callbacks are registered', () {
        expect(() => listeners.notifySignal(_signal(), 'peer-1'), returnsNormally);
      });
      ...
      test('does not call a sender-specific callback for a non-matching '
          'sender', () {
        var called = false;
        listeners.onSignal((_) => called = true, 'peer-a');
        listeners.notifySignal(_signal(), 'peer-b');
        expect(called, isFalse);
      });

      test('wildcard callback (from=null) fires for any sender', () {
        final received = <ISignalErmes>[];
        listeners.onSignal(received.add, null);
        listeners.notifySignal(_signal(), 'peer-a');
        listeners.notifySignal(_signal(), 'peer-b');
        listeners.notifySignal(_signal(), null);
